...
- Перешли на серверное хранение B2 кредов через мастер-ключ (см. `план_B2.md` для деталей).
- Клиентская ключ-фраза для файлов убирается (загрузка/скачивание без пользовательского AES).
- Если документов нет и нет B2 настроек — загрузка запрещена (гейт): нужно добавить B2 креды в Settings.
- Все файлы/превью отправляются на B2 только в зашифрованном виде (AES-GCM на клиенте). Незашифрованных файлов быть не должно; их появление — ошибка.
