...
[ ] Перенести UI, связанный с master key/backup phrase, из `/upload` в `/settings` (оставить на upload только блокирующее сообщение и ссылку в Settings).
[ ] Сделать UX ключа «настойчивым»: явное подтверждение сохранения backup phrase, предупреждение до закрытия/ухода без подтверждения, понятный recovery-сценарий.
